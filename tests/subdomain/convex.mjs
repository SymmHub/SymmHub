/*
  ...
  domain of 442, and of the hyperbolic triangle groups *237 and *245:
  ...
  2. the exact convexity test agrees with the sampling test of
     checkDomainConvex() on every Euclidean union (both choices)
  3. a found union is convex, and it is a fundamental domain: the image of
     a point and of its H-translate coincide (subgroupImage), and the fold
     of random points reaches every cell
...
import { isUnionConvex } from '../../lib/grouplib/SubgroupConvex.js';
...
for(const [label, angles, relators] of [
  ...
  CASES.push({ label, group: new Group({ s, t: s.map(sp => [sp]) }), presentation: { gens: 'a b c', relators },
               maxIndex: Math.min(MAX_INDEX, 6), radius: 0.85, euclidean: false, maxIterations: 3000 });
}
...
for(const c of CASES){
  ...
  for(const sub of data.subgroups){
    ...
    // 2. the exact test against the sampling test
    if(c.euclidean){
      for(const [d, what] of [[plain, 'growth'], [conv, 'search']]){
        const s = checkDomainConvex({ group: c.group, domain: d, radius: 1.0, step: 0.02 });
        if(s.convex !== d.isConvex){ disagree++; console.log(`  ${tag} ${what}: exact says ${d.isConvex}, sampling says ${s.convex} (${s.unionOnly}/${s.halfPlaneOnly})`); }
      }
    }
