...
export class iSplane {
  ...
  derivative(point)// a point presumed to be a point splane
  {
    if(!point.type || point.type!=SPLANE_POINT){return}
    switch(this.type){
      case SPLANE_NONE: return 1;
      case SPLANE_PLANE: return -1;
      case SPLANE_POINT: return -1; // this is backwards in 2D, but.
      case SPLANE_SPHERE: 
        var pcoords = point.v;
        var scoords = this.v;
        return  (scoords[3]*scoords[3])/(
          (pcoords[0]-scoords[0])*(pcoords[0]-scoords[0])+
          (pcoords[1]-scoords[1])*(pcoords[1]-scoords[1])+
          (pcoords[2]-scoords[2])*(pcoords[2]-scoords[2]));
    }
  }
