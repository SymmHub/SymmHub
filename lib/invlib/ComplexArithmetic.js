...
// a transform from a point to the origin, adding in a homothety (complexscale)
 ...
 export function transformFromCenterToPoint(center,complexscale,curvature=-1,debug=false){
    ...
    //////////////
    //
    // Calculate imagetransform from the origin TO the center of a placed image, with a
    // homothety (rotate and scale by multiplying by a complex number expressed as[real part, im part]).

    // The output is a list of splanes.

    var cc=0,ss=1,texturewidth=1;
    cc = complexscale[0]; //angle and scale x
    ss = complexscale[1]; //angle and scale y
    texturewidth= Math.sqrt(cc*cc+ss*ss);

    imagetransform = [
        new iSplane({v:[0,0,0,1],type:SPLANE_SPHERE}),
        new iSplane({v:[0,0,0,Math.sqrt(texturewidth)],type:SPLANE_SPHERE})];
        // so far, the scaling.,
        v1b,vd];

    cc = cc/texturewidth;// unscaled cosine
    ss = ss/texturewidth;// unscaled sine


    // we want to rotate by the angle of cc and ss, but to do this with two 
    // reflections. One of the reflections will be the vertical axis; the other 
    // will be a mirror at _half_ of this angle. Thus we use the half-angle formulas.
    //  cosine is always now positive, 

    cc = Math.sqrt(.5*(1+cc));  
    
    // but the sign of sine
    ...
    if(center[0]!=0 || center[1]!=0){
        ...
        imagetransform = [v1b,vd,v2a,v2b];
    }
    else{// if the center is at zero
      imagetransform=[v1b,vd]}// just rotate 

    //precompose the imagetransform with the imagescaling.

    imagetransform = iGetFactorizationU4(scalingtransform.concat());
...
//
//  return product of 2 complex numbers 
//
