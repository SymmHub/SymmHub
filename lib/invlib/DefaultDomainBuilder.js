...
export class DefaultDomainBuilder {
  ...
  constructor(options){
    ...
    this.patternMaker = getParam(opt.patternMaker,false);
  ...
  */
  ...
  getUniforms(un, timeStamp){
    ...
    this.calculateGroup();
    var group = this.group;// WallPaperGroup_General FD={s sides,t generating transforms,i interiors, c crown transforms}
    var fd = group.s; // the sides of the fundamental domain
    var trans = group.t; // a list of generators 
      // we need to pass in the bounds of the texture that we'll sample.
    var ctrans = group.c; // a list of transforms used in creating the crown
    ...
    // domain data is used for generating the group, drawing the fundamental domain, etc.
    // (Each wall of the fundamental domain is a mirror or has a matching wall, generating the group)
    ...
    // u_groupTransformsData is a list of generators for the group
    ...
    // u_cTransformsData is a list of specific transforms for the crown
    un.u_cTransCumRefCount = iPackRefCumulativeCount(ctrans, this.MAX_GEN_COUNT);// NOTE: This has changed!!
    un.u_cTransformsData = iCumPackTransforms(ctrans, this.MAX_GEN_COUNT, this.MAX_TOTAL_REF_COUNT);
  ...
  render(context, transform){
    ...
    //draw the fundamental domain and other stuff in javascript
    var par = this.params;
  ...
  calculateGroup(){
    ...
    this.groupMaker["patternMaker"]=this["patternMaker"]; //for access to the textures for working out the crown.
