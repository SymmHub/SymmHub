...
uniform float uFDoutlineThickness;
...
void main () {
    ...
    if(uLsEnabled){
        float lsDist = lensHeight / scale;
        float lsValue = isoline(lsDist, 0., 2.*uLsThickness);
        color = mask*overlayColor(color, lsValue * uLsColor);                
    }  
    ...
    if(uGensEnabled){
        ...
        for(int gindex = 0; gindex < domainSize; gindex++){
            ...
            float v = iDistance(sp, vec3(pnt, 0.));	    
            float dens = isoline_with_shadow(v, 0., uGensWidth, uGensShadowsWidth, 0.5);
            //float dens = isoline_v0(v, 0.,uGensWidth);
            genDens = max(genDens, dens);
        }
    ...
    if(uTilingEnabled){
        ...
        for(int gindex = 0; gindex < domainSize; gindex++){
            iSPlane sp = getSplane(uGroupData, 0, gindex);
            float v = iDistance(sp, vec3(pfd, 0.));	    
            float dens = isoline_v0(v, 0.,uTilingWidth);
