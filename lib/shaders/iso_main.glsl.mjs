...
void main () {
    ...
    if(uGridEnabled) {
        ...
        float rr = log(length(pnt));
        float angle = atan(pnt.y, pnt.x)/PI;
        float gridDensX = isolines_multi(rr, uIsoOffset, uGridStep.x, uIsoThickness, uIsoLevels);
        float gridDensY = isolines_multi(angle, uIsoOffset, uGridStep.y, uIsoThickness, uIsoLevels);
