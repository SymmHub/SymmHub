 ...
 *
 * Interaction:
 *  - the edges of the fundamental domain, and optionally its interior, are
 *    drawn on the WebGL pattern canvas (or, without a glEdgeRenderer, the
 *    edges alone on the 2D overlay canvas);
 ...
 *  - SymRenderer overlay:     renderUI(ctx, canvasTransform); when
 *    options.glEdgeRenderer (an OrbifoldEdgeGLRenderer) was given to the
 *    constructor, the edges are drawn with it directly onto the WebGL
 *    pattern canvas instead of the 2D ctx (ctx is then unused — it is still
 *    accepted so the call site doesn't need to special-case this tool);
...
import {
    ...
    nearArcQ, iDrawSplane, getCopy, iSphere,
    SHORTEPSILON,
    sign, abs, cos, sin,
    complexN,
...
export class OrbifoldUITool {
     ...
     *               config.showFill the domain interior
     *   styles    - optional style overrides
     *   glEdgeRenderer - optional OrbifoldEdgeGLRenderer; when given, the fd
     *                    edges and interior are drawn directly on the WebGL
     *                    pattern canvas instead of the 2D overlay canvas (ctx
     *                    is then unused)
     */
    constructor(options = {}) {
        ...
        this.glEdgeRenderer = options.glEdgeRenderer || null;
    ...
    //
    // the splanes bounding the domain (fd.s only — fd.i are interior edges,
    // which cut the domain up rather than bound it)
    //
    getBoundarySplanes() {
        const splanes = this.wg.getGroup().s.map(face => face[0]);
        // The edge splanes bound the domain within the hyperbolic plane only:
        // taken as splanes of the whole inversive plane their common solid side
        // runs on past the circle at infinity, so the disk has to bound it too.
        // (The navigator's transform preserves the disk, so this splane is the
        // same in view coordinates as it is here.)
        splanes.push(iSphere([0, 0, 0, 1]));
        return splanes;
    }
    ...
    renderUI(ctx, canvasTransform) {
        ...
        const drawSplane = this.glEdgeRenderer
            ? (splane, param) => this.glEdgeRenderer.drawSplane(
                  splane, this.navigator.getInversiveTransform(), canvasTransform, param)
            : (splane, param) => iDrawSplane(splane, ctx, this.navigator, param);

        // the domain interior, under everything else (GL only — the 2D path
        // has no way to fill the intersection of a set of splanes)
        if (this.glEdgeRenderer && this.config.showFill) {
            this.glEdgeRenderer.drawInterior(
                this.getBoundarySplanes(),
                this.navigator.getInversiveTransform(),
                canvasTransform,
                { fillStyle: this.styles.interior.color });
        }

        // highlight under the active edge
        if (ap >= 0 && parts[ap] && parts[ap][0]) {
            const st = this.styles.activeColor;
            drawSplane(parts[ap][0], {
                lineStyle: st.color,
