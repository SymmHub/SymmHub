...
export function wallpaperPresentation(orbifold) {
  ...
  return { name: key, family: 'wallpaper', geometry: 'euclidean', dimension: 2, ...p };
}

/**
 * Which plane a triangle group tiles: 1/k + 1/l + 1/m against 1, in integers
 * so that (3,3,3) does not miss Euclidean by a rounding error.  Only the
 * Euclidean ones have a translation lattice, hence a dimension.
 */
export function triangleGeometry(k, l, m) {
  const lhs = l * m + k * m + k * l, rhs = k * l * m;
...
export function klmPresentation(k, l, m) {
  const geometry = triangleGeometry(k, l, m);
  return {
    name: `${k}${l}${m}`,
    family: 'klm',
    geometry,
    ...(geometry === 'euclidean' ? { dimension: 2 } : {}),
    gens: 'a b c d',
