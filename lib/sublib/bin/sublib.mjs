...
import {
  ...
  subgroupStructure, ellipticElements,
} from '../src/sublib.js';
...
options
  ...
  --structure ID       print what a subgroup contains: elliptic elements,
                       homology, whether it is torsion-free or all translations
  --simplest           find the least-index subgroup with no elliptic elements,
                       and the least-index one made of translations alone
...
} else if (flag('--structure') !== undefined) {
  const want = flag('--structure');
  const id = /^\d+$/.test(want) ? Number(want) : want;
  const entry = typeof id === 'number' ? data.subgroups[id] : data.subgroups.find(s => s.subgroup === id);
  if (!entry) { console.error(`no subgroup "${want}" — ids look like ${data.subgroups[1]?.subgroup}`); process.exit(1); }
  const st = subgroupStructure(data, entry);
  console.log(`${entry.subgroup}  index ${entry.index}`);
  console.log(`  elliptic elements : ${st.torsionFree ? 'none — torsion-free' : st.orders.join(', ')}`);
  if (st.abelianization) console.log(`  homology          : ${st.abelianization.text}`);
  if (st.translationsOnly !== undefined) {
    console.log(`  translations only : ${st.translationsOnly ? 'yes' : 'no'}`);
  }
  for (const t of st.torsion) {
    console.log(`    order ${String(t.order).padStart(2)}  ${t.word.padEnd(18)}` +
      `= ${t.from.word}^${t.cycleLength} carried to coset ${t.coset}`);
  }
} else if (has('--simplest')) {
  const elliptics = ellipticElements(data.presentation);
  console.log(`${data.name}  searched to index ${data.maxIndex}` +
    `  (elliptic generators: ${elliptics.map(e => e.word + '^' + e.order).join(' ') || 'none'})`);
  let tf = null, tr = null;
  for (const s of data.subgroups) {
    const st = subgroupStructure(data, s.subgroup);
    if (!tf && st.torsionFree) tf = st;
    if (!tr && st.translationsOnly) tr = st;
    if (tf && tr) break;
  }
  const line = (label, st) => console.log(`  ${label.padEnd(22)}: ` + (st
    ? `${st.subgroup.padEnd(14)} index ${String(st.index).padStart(3)}   H1 ${st.abelianization ? st.abelianization.text : '?'}`
    : `none of index <= ${data.maxIndex}`));
  line('no elliptic elements', tf);
  if (data.dimension != null) line('translations only', tr);
  else console.log('  translations only     : not a Euclidean group — no translation lattice to find');
