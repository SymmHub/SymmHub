...
function ParamChoice(arg) {
    ...
    function updateChoices(newChoices) {
        ...
        // an unchanged list leaves the select alone: rebuilding its options
        // resets it to the first one, and while the user is editing the select
        // (its own onChange) dat.gui does not refresh the display
        const same = newChoices.length === choice.length && newChoices.every((c, i) => c === choice[i]);
        choice = newChoices;
        if (!same && control && typeof control.updateOptions === 'function') {
        ...
        for(var key in values){
            ...
            if(isDefined(param)){
                ...
                if(isDefined(param.setValue)){
                    ...
                    // initialize goes down the tree: a document load (initialize)
                    // replaces nested lists, a patch (no initialize) edits them
                    param.setValue(values[key], initialize);
                }
            }
        }
    }
