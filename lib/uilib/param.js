...
function ParamObjArray(arg) {
    ...
    function createObjUI(folder, obj) {
        if (isDefined(obj.getParams)) {
            createParamUI(folder, obj.getParams());
        } else if (isDefined(obj.createUI)) {
            obj.createUI(folder);
        } else if (isDefined(obj.initGUI)) {
            obj.initGUI({ folder });
        } else {
            console.warn(`ParamObjArray: item has no getParams/createUI/initGUI`, obj);
        }
    }
    ...
    function createUI(gui) {
        ...
        mItems.forEach(({ name, obj }) => {
            const itemFolder = folder.addFolder(name);
            createObjUI(itemFolder, obj);
        });
    }

    // ── serialization — delegate to ObjArray ──────────────────────────────────
