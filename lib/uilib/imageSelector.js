...
function createImageSelector(param = {}){
    ...
    const bgStorageKey = param.storageId ? (param.storageId + '_bgMode') : 'fileDialog_bgMode';
    let mBgMode = localStorage.getItem(bgStorageKey) ?? 'checkerboard';

    function setBgMode(mode) {
        mBgMode = mode;
        localStorage.setItem(bgStorageKey, mode);
        // Apply background mode to all current non-folder thumbnails
        div.querySelectorAll('.thumbnail-image').forEach(img => {
            if (!img.classList.contains('folder-thumbnail')) {
                img.classList.remove('bg-lightgray', 'bg-darkgray', 'bg-checkerboard');
                img.classList.add('bg-' + mode);
            }
        });
    }
    ...
    function addFromTmb(tmb, userData){
        ...
        if (!(userData && userData.isFolder)) {
            item.elem.querySelector('.thumbnail-image')?.classList.add('bg-' + mBgMode);
        }
