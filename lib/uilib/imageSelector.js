...
//  
//  param.onSelect - call back when image is selected (single click)
//  param.onDoubleClick - call back when image is double clicked (opened) 
...
function createImageSelector(param = {}){
    ...
    let onDoubleClick = param.onDoubleClick || null;
    ...
    function addFromTmb(tmb, userData){
        ...
        let item = createImageItemElem({tmb: tmb, onClick:onImageClick, onContextMenu:onImageContextMenu, onDoubleClick:onImageDoubleClick, userData: userData});        
    ...
    function addFrpomURL(imgUrl, userData){
        ...
        let item = createImageItemElem({url: imgUrl, onClick:onImageClick, onContextMenu:onImageContextMenu, onDoubleClick:onImageDoubleClick, userData: userData});        
        if (!(userData && userData.isFolder)) {
            const img = item.elem.querySelector('.thumbnail-image');
            if (img) applyThumbnailBg(img);
        }
    ...
    function addFromFile(file, userData){
        ...
        let item = createImageItemElem({file:file, onClick:onImageClick, onContextMenu:onImageContextMenu, onDoubleClick:onImageDoubleClick, userData: userData, tmbSize: tmbSize});        
        if (!(userData && userData.isFolder)) {
            const img = item.elem.querySelector('.thumbnail-image');
            if (img) applyThumbnailBg(img);
        }
    ...
    //
    //  double click on an item opens it (enters a folder, loads a file);
    //  the clicks that precede the dblclick event have selected it already
    //
    function onImageDoubleClick(imgItemElem){
        if(DEBUG)console.log(MYNAME + '.onImageDoubleClick():', imgItemElem);
        if (onDoubleClick) {
            onDoubleClick(imgItemElem.getUserData());
        }
    }
    ...
    function onImageContextMenu(imgItemElem, event){
        ...
        if (onContextMenu) {
            // the menu acts on this item: show that by selecting it
            selectItem(imgItemElem);
...
function createImageItemElem(options){
    ...
    let onDoubleClick = options.onDoubleClick;
    ...
    // click, double click and the context menu act on the whole tile,
    // the caption included
    container.addEventListener("click", myOnClick);
    if (onDoubleClick) {
        container.addEventListener("dblclick", myOnDoubleClick);
    }
    if (onContextMenu) {
    ...
    function myOnDoubleClick(){
        if(DEBUG)console.log(MYNAME + '.onDoubleClick() userData: ', userData);
        if(onDoubleClick) onDoubleClick(myself);
    }
    ...
    //
    //  the text under the image, and the tooltip of the tile
    //
