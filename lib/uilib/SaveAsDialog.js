...
export function createSaveAsDialog(options = {}) {
    ...
    folderLabel.style.cssText = [
        ...
        'direction:rtl',   // clip left edge → deepest folder always visible
    ].join('; ');
    folderLabel.textContent = '(none selected)';
    ...
    function setFolder(handle, displayPath = null) {
        ...
        folderLabel.title = fullText;   // always available on hover

        // Prepend U+200E (LRM — Left-to-Right Mark): an invisible strong LTR character.
        // This anchors the string as LTR for the bidi algorithm, preventing the
        // browser from reordering segments when the path starts with numbers (e.g.
        // "250125_symhub / samples"). direction:rtl on the element then correctly
        // clips from the left, keeping the deepest folder name visible.
        folderLabel.textContent = '\u200E' + fullText;
        folderLabel.style.color = '#444';
    }
    ...
    function onChangeFolderClick() {
        ...
        if (!mFolderPicker) {
            mFolderPicker = createFileSelectionDialog({
                ...
                left:           '240px',
                top:            '100px',
                // onFolderChange fires on every navigation; live-updates the SaveAsDialog folder.
                // The folder picker (FileSelectionDialog) saves its own state to IDB — no need
                // to also save here.
        ...
        // Show constrained to the root folder (prevents navigating above it)
    ...
    async function show({ suggestedName = '', suggestedHandle = null, suggestedPath = null,
        ...
        // Use the folder provided by the caller (from FileSelectionDialog's current folder).
        // No IDB fallback here — that was causing stale values from previous sessions to appear.
        // The embedded folder picker remembers its own last location via its own IDB.
        const displayPath = (suggestedHandle && suggestedPath) ? suggestedPath : null;
        setFolder(suggestedHandle, displayPath);
