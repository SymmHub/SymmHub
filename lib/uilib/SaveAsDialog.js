...
export function createSaveAsDialog(options = {}) {
    ...
    folderLabel.style.cssText = 'flex:1; min-width:0; font-size:12px; color:#444; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;';
    ...
    function setFolder(handle, displayPath = null) {
        ...
        folderLabel.textContent = displayPath ?? (handle ? handle.name : '(none selected)');
    ...
    function onChangeFolderClick() {
        ...
        if (!mFolderPicker) {
            mFolderPicker = createFileSelectionDialog({
                ...
                onFolderChange: async (handle, path) => {
                    setFolder(handle, path);
                    await saveHandleToIDB(idbKey, handle);
                },
            });
        }
    ...
    async function show({ suggestedName = '', suggestedHandle = null, suggestedPath = null,
        ...
        // Resolve folder: caller suggestion → IDB → null
        const folder = suggestedHandle ?? await loadHandleFromIDB(idbKey);
        // Use the provided path string when showing a folder from FileSelectionDialog;
        // fall back to handle.name when a handle was restored from IDB (no path known).
        const displayPath = (suggestedHandle && suggestedPath) ? suggestedPath : null;
        setFolder(folder, displayPath);
