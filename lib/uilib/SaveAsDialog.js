...
export function createSaveAsDialog(options = {}) {
    ...
    async function show({ suggestedName = '', suggestedHandle = null, suggestedPath = null,
        ...
        // Prefer caller-supplied handle (e.g. from the open FileSelectionDialog);
        // fall back to the handle we persisted to IDB after the last successful save
        // so the dialog always opens with a sensible folder, even on first launch or
        // when the user has never opened FileSelectionDialog in this session.
