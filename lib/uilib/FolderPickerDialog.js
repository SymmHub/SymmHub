...
export function createFolderPickerDialog(options = {}) {
    ...
    interior.style.flexDirection = 'column';
    interior.style.overflow = 'hidden';
    interior.style.cursor = 'default';
    ...
    function renderFolderNode(dirHandle, pathStr, isRoot = false) {
        ...
        // Toggle arrow (custom unicode caret)
        const arrow = document.createElement('span');
        arrow.style.cssText = 'width:16px; height:16px; display:inline-flex; align-items:center; justify-content:center; font-size:12px; color:#666; cursor:pointer;';
        arrow.textContent = '\u25B8'; // ▶
        row.appendChild(arrow);

        // Folder Icon
        const icon = document.createElement('span');
        icon.style.cssText = 'font-size:14px; margin-right: 2px;';
        icon.textContent = '\ud83d\udcc1'; // 📁
        ...
        row.addEventListener('mouseenter', () => {
            if (mSelectedRow !== row && !row.classList.contains('selected-node')) row.style.background = '#f1f3f5';
        });
        row.addEventListener('mouseleave', () => {
            if (mSelectedRow !== row && !row.classList.contains('selected-node')) row.style.background = '';
        });
        ...
        // Toggle expansion handler (custom unicode caret)
