...
import { DatGUI, ParamFunc, ParamGroup } from './modules.js';
...
export function createFileSelectionDialog(options = {}) {
    ...
    // Thumbnail BG mode selection group
    const bgGroup = ParamGroup({
        name: 'Thumbnail BG',
        params: {
            lightgray: ParamFunc({
                name: 'Light Gray',
            ...
            darkgray: ParamFunc({
                ...
                func: () => {
                    mMenuEl.style.display = 'none';
                    mSelector.setBgMode('darkgray');
                }
            }),
            checkerboard: ParamFunc({
                name: 'Checkerboard',
                func: () => {
                    mMenuEl.style.display = 'none';
                    mSelector.setBgMode('checkerboard');
                }
            })
        }
    });
    bgGroup.createUI(mMenuGUI);
