...
import { DatGUI, ParamFunc    } from './modules.js';
...
export function createFileSelectionDialog(options = {}) {
    ...
    const mMenuGUI = new DatGUI({ autoPlace: false, hideable: false, width: 200 });
    ...
    const mTitleEl = mSelector.getTitleEl?.();
    if (mHeader) {
        ...
        if (mTitleEl) {
            mTitleEl.style.flex        = '1';
            mTitleEl.style.minWidth    = '0';
            mTitleEl.style.width       = '';   // clear fixed calc() width — flex handles it
            mTitleEl.style.marginRight = 'calc(var(--drag-header-size) + 4px)';
        }
    ...
    async function show(showOpts = {}) {
        ...
        } else if (mDirHandle !== mPopulatedHandle) {
            // Folder changed since last populate (e.g. restored subfolder not yet shown)
            await populateFromFolder(mDirHandle);
        }
