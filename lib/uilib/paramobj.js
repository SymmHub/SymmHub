...
function ParamObjArray(arg) {
    ...
    function addChild(className, insertIdx = mParamObjs.length) {
        ...
        if (mFolder) {
            ...
            entry.paramObj.createUI(mFolder);
            const next = mParamObjs[insertIdx + 1];
            const li_new  = entry.paramObj.getFolder()?.domElement?.parentElement;
            const li_next = next && next.paramObj.getFolder()?.domElement?.parentElement;
            if (li_new && li_next && mFolder.__ul) mFolder.__ul.insertBefore(li_new, li_next);
            const getIdx = () => mParamObjs.indexOf(entry);
            const btns = injectListButtons(entry.paramObj, {
                onMoveUp:   () => moveChildAt(getIdx(), -1),
                onMoveDown: () => moveChildAt(getIdx(),  1),
    ...
    function moveChildAt(i, delta) {
        ...
        // Capture DOM li's before the array swap (none before createUI).
        const f_i = mParamObjs[i].paramObj.getFolder();
        const f_j = mParamObjs[j].paramObj.getFolder();
        const li_i = f_i && f_i.domElement.parentElement;
        const li_j = f_j && f_j.domElement.parentElement;
    ...
    return { createUI, getValue, setValue, init, getItems, addChild, removeChildAt, moveChildAt, renameAll };
