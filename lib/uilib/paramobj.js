...
function ParamObj(arg) {
    ...
    function setName(newName) {
        ...
        // Skip rename if a different sibling folder already has this name,
        // to prevent __folders map collisions from user-triggered id edits.
...
function ParamObjArray(arg) {
    ...
    function setValue(value, initialize = false) {
        ...
        // ── Phase 1: shrink — remove extra children from the end ─────────────
        while (mParamObjs.length > childArray.length) {
            const i = mParamObjs.length - 1;
            const entry = mParamObjs[i];
            if (mFolder && entry.paramObj.getFolder())
                mFolder.removeFolder(entry.paramObj.getFolder());
            mParamObjs.splice(i, 1);
            mItems.splice(i, 1);
            mObjArray.removeChild(i);
        }

        // ── Phase 2: grow — append new children for incoming extras ──────────
        while (mParamObjs.length < childArray.length) {
            const i        = mParamObjs.length;
            ...
            mObjArray.addChild(newObj, i);
            mItems.splice(i, 0, { name: indexedFolderName(i, newObj), obj: newObj });

            const entry = makeEntry(newObj, i);
            mParamObjs.splice(i, 0, entry);
