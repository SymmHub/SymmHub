...
function ParamObjArray(arg) {
    ...
    function setValue(value, initialize = false) {
        ...
        // ── Phase 1+2: reorder-and-patch ────────────────────────────────────
        //
        // The old two-pass approach (shrink → grow) left surviving layers in
        // their original order.  Phase 2 then appended new layers at the tail,
        // so the live array order never matched the incoming JSON order.
        // Phase 3 positional assignment therefore wrote the wrong values into
        // each slot (e.g. overlay's params into bumpmap's slot), and the final
        // renameAll() stamped wrong ids onto the folders.
        //
        // New single-pass algorithm:
        //  1. Build a pool of currently-live { mItems[i], mParamObjs[i] } pairs,
        //     keyed by their id.
        //  2. Walk the incoming childArray in order.  For each incoming entry:
        //     a. If a live entry with a matching id exists in the pool, claim it.
        //     b. Otherwise, grow a new object via the factory.
        //  3. Any pool entries not claimed (ids absent from incoming JSON) are
        //     removed from the UI and discarded.
        //  4. Replace mItems and mParamObjs in-place with the new ordered arrays.
        //
        // This guarantees that after this phase the live arrays are in exactly
        // the same order as childArray, so Phase 3 positional assignment is correct.
        {
            // Build id-keyed pool from current live entries.
            // Entries without an id are pooled under null and claimed FIFO.
            const poolById  = new Map();   // id → [{ item, entry }, ...]
            const poolNoId  = [];          // entries with no id (FIFO)
            ...
            for (let i = 0; i < childArray.length; i++) {
                const childVal  = childArray[i];
                const wantedId  = childVal?.params?.id ?? null;
                const className =
                    (childVal && childVal.className) ||
                    (mFactory && mFactory.getDefaultName());

                // Try to claim a matching live entry from the pool.
                let claimed = null;
                if (wantedId && poolById.has(wantedId)) {
                    const bucket = poolById.get(wantedId);
                    claimed = bucket.shift();
                    if (bucket.length === 0) poolById.delete(wantedId);
                ...
                if (claimed) {
                    ...
                    // Grow: create a new object via the factory.
                    if (!className || !mFactory) {
                        console.warn('ParamObjArray.setValue(): cannot grow — no factory or className');
                        break;
                    }
                    const newObj = mFactory.getObject(className);
                    if (!newObj) break;

                    const assignedId = newObj.getId ? newObj.getId() : '?';
                    if (assignedId !== wantedId) {
                        console.warn(
                            `[ParamObjArray] GROW: factory assigned id="${assignedId}"`,
                            `but JSON wants id="${wantedId}" (class=${className})`,
                            `| existing ids before grow: [${mItems.map(it => it.obj?.getId?.()).join(', ')}]`,
                        );
                    } else {
                        console.log(
                            `[ParamObjArray] GROW: added layer[${i}] id="${assignedId}" class=${className}`,
                        );
                    }
            ...
            // Remove unclaimed live entries (ids absent from incoming JSON).
            const allUnclaimed = [
                ...poolNoId,
                ...[...poolById.values()].flat(),
            ];
            for (const bucket of allUnclaimed) {
                const removedId = bucket.item?.obj?.getId?.() ?? '(unknown)';
                console.warn(`[ParamObjArray] REORDER: discarding unclaimed layer id="${removedId}"`);
                if (mFolder && bucket.entry.paramObj.getFolder())
                    mFolder.removeFolder(bucket.entry.paramObj.getFolder());
            }

            // Rebuild mObjArray.mChildren to match the new order by replacing
            // the backing array in-place via removeChild/addChild operations.
            // Use a simple clear-and-refill approach on the live mObjArray.
            const targetObjs = newItems.map(it => it.obj);
            // Remove all from tail to head to avoid index shifting issues.
            for (let i = mParamObjs.length - 1; i >= 0; i--) {
                mObjArray.removeChild(i);
            }
            // Add back in the correct order.
            targetObjs.forEach((obj, idx) => mObjArray.addChild(obj, idx));

            // Replace parallel arrays in-place.
            mItems.length     = 0;
            mParamObjs.length = 0;
        ...
        // ── Phase 2.5: pre-rename all retained folders to guaranteed-unique temp
        //   names before applying values.  This prevents onIdChange callbacks fired
        //   during setValue (Phase 3) from seeing stale sibling names and either
        //   bailing on a false collision or corrupting __folders tracking.
        //   (renameAll() does the same thing afterwards, but too late.)
        const TMP_PRE = '\x00__pre__';
        mParamObjs.forEach((entry, i) => entry.paramObj.setName(TMP_PRE + i));

        // ── Phase 3: apply values positionally ───────────────────────────────
            ...
            if (mFactory && childValue && childValue.className) {
                ...
                if (currentClass !== childValue.className) {
                    ...
                    if (newObj) {
                        ...
                        mItems[i].obj = newObj;
                        mObjArray.replaceChild(i, newObj);  // keep mLayerArray in sync
