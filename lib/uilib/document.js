    ...
    function createDocument(docData = {}){
        ...
        let myself = {
            ...
            async writeParamsTo(folderHandle, baseName) {
                ...
                if (getUseBinaryFiles()) {
                    ...
                    await writeFile(folderHandle, fileName, getParamsAsJSON(baseName));
                }
            },
        };
        ...
        async function saveDocTo(name){
            ...
            if (getUseBinaryFiles()) {
                ...
                if (store.getChunkCount() > 0) {
                    // At least one ParamCustom registered binary data — write sidecar.
                    const manifest = store.toManifest(binName);
                    const pset = {
                        name:        name,
                        appInfo:     mAppInfo,
                    ...
                    // No binary chunks — save as plain JSON (no sidecar, no binary_data key).
