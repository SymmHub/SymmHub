...
// the sublib catalogue holds the presentation of every group's default domain,
// and the shipped subgroup tables were made from it: keep using it there
const USE_CATALOGUE_FOR_DEFAULT_DOMAIN = true;
...
export class Group_WP {
	...
	constructor(opt){
        ...
        const type = getParam(opt.type, '333');
        this.params = {
          type:     type,
          // shape of the fundamental domain, one of getWallpaperDomainShapes(type)
          domainShape: resolveWallpaperDomainShape(type, opt.domainShape),
  ...
  */
  ...
  //
  // called from UI when the group type was changed 
  //
  onTypeChanged(){
    // the list of domain shapes depends on the group: keep the current shape
    // if the new group offers it, fall back to the group's default shape otherwise
    const par = this.params;
    par.domainShape = resolveWallpaperDomainShape(par.type, par.domainShape);
    if(this.mParams){
      this.mParams.domainShape.updateChoices(getWallpaperDomainShapes(par.type));
      this.mParams.domainShape.updateDisplay();
    }
    this.onParamChanged();
  }
  ...
  */
    ...
    //
    //  presentation of the group as the current domain gives it: the generators
    //  are the sides, the relators come from the side pairings and the corner
    //  cycles (see GroupPresentation.js).  The subgroup enumeration works from it.
    //
    //  Presentation of the group as the maker gives it: {name, domainShape,
    //  label, gens, relators} for a domain shape other than the default.  For
    //  the default domain the catalogue applies: {name, domainShape, label,
    //  preset} without gens and relators, preset being the sublib key of the
    //  group ('wallpaper:632'); the subgroup tables of the catalogue match the
    //  default domain (tests/wallpaper/presentation.mjs).
    //
    getPresentation(){
        const pm = this.params;
        if(USE_CATALOGUE_FOR_DEFAULT_DOMAIN && pm.domainShape === getWallpaperDomainShapes(pm.type)[0]){
            const value = { name: pm.type, domainShape: pm.domainShape, label: pm.type };
            if(pm.type !== 'trivial') value.preset = 'wallpaper:' + pm.type;   // sublib knows the 17 groups, not the trivial one
            return value;
        }
        const key = pm.type + '|' + pm.domainShape;
        if(!this.mPresentation || this.mPresentation.key !== key){
            let value = null;
            try {
                const pres = groupPresentation(this.getGroup());
                value = {
                    name:        pm.type,
                    domainShape: pm.domainShape,
                    label:       pm.type + ' [' + pm.domainShape + ']',
                    gens:        pres.gens,
                    relators:    pres.relators,
                };
            } catch(e){
                console.warn(MYNAME + '.getPresentation(): ' + e.message);
            }
            this.mPresentation = {key: key, value: value};
        }
        return this.mPresentation.value;
    }
        ...
        return {
          ...
          domainShape:
                ParamChoice({
                      ...
                      key:  'domainShape',
                      name: 'domain shape',
                      choice:   getWallpaperDomainShapes(par.type),
                      onChange: opc,
                }),
           a: 
                ParamFloat({
