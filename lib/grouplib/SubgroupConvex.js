...
/*
  ...
  intersection of the inner half spaces of its boundary splanes.  For a
  union of cells that is the case exactly when every cell whose interior
  meets the interior of the convex hull of the union belongs to the union.
  The hull is taken in the model where the splanes are straight: the plane
  itself for a Euclidean group, the Klein model for a group of the Poincare
  disk (the map k = 2p/(1+|p|^2) sends every geodesic to a chord, so the
  hyperbolic hull is the Euclidean hull of the mapped corners).  Spherical
  groups are not handled (null).

  The search grows connected unions from F and closes each one under the
  rule above: a union which is part of a convex fundamental domain has its
  closure in that domain too.  A closure with two cells of the same class,
  or with more than n cells, cannot be completed; a closure of n cells of
  distinct classes is a convex fundamental domain.  The walk is depth first
  over closed unions, each extended by one neighbouring cell of an unclaimed
  class, nearest to F first, with a budget of closures; among the solutions
  the most compact one is taken (the least radius about F, then the least
  perimeter, then the shortest words).
*/
...
/** 'euclidean' (straight sides), 'hyperbolic' (circle sides orthogonal to the unit circle), else null */
...
function makeContext(group, perms, geometry, corners){
  ...
  function makeCell(itrans, cls, word){
    ...
    const poly = corners.map(p => {
      ...
      return toModel([q[0], q[1]], geometry);
    });
    if(polygonArea(poly) < 0) poly.reverse();
    c = { key, itrans, cls, word, poly, area: polygonArea(poly), anchor: centroid(poly) };
    cache.set(key, c);
    return c;
  }
  ...
  return { n: perms ? perms[0].length : Infinity, nsides: group.getFundDomain().length, makeCell, neighbor };
}
...
/**
  ...
  return null when the geometry is not handled, or nothing was found within
  the budgets; else {
    ...
    geometry, states, solutions, radius, perimeter, exhausted (the budget ran out)
  }
*/
...
/**
  is the union of the cells convex?  Exactly when no cell outside the union
  reaches the interior of its hull: the closure may not add a cell.  The cells
  are given by their transforms (as buildSubgroupDomain returns them); false
  as well when the geometry is not handled
*/
