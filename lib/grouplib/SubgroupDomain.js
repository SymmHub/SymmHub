...
import { findConvexTransversal, isUnionConvex } from './SubgroupConvex.js';
...
/**
  ...
  opt = {
    ...
    convex: look for a convex union of cells first, or for the union with the
            least hull excess when there is none (default true, see
            SubgroupConvex.js); the compact growth is the fallback
    maxStates, maxUnions, bestEffort: budgets and the fallback of that search
  }

  return {
    n:          index of H in G (count of cosets = count of cells)
    isConvex:   whether the union of the cells is convex (SubgroupConvex.isUnionConvex)
    convexSearch: {found, bestEffort, excess, states, solutions, unions, exhausted, geometry}
                of the search, null when skipped
    ...
    sides:      array of {cell, side, splane, kind:'interior'|'boundary',
                  neighborClass,            // class (cell index) of the cell across the wall
...
export function buildSubgroupDomain(opt){
  ...
  // ---- step 1b: a convex choice of cells, when there is one -------------
  //
  //  Compact growth keeps the union disc-like, not convex.  The search of
  //  SubgroupConvex.js looks for cells whose union is convex (the domain the
  //  point reduction needs), or, when there is none, for the union with the
  //  least hull excess; the coset transversal above stays as it is.
  let convexSearch = null;
  if(opt.convex !== false){
    let found = null;
    try {
      found = findConvexTransversal({ group, perms, maxStates: opt.maxStates, maxUnions: opt.maxUnions,
                                      bestEffort: opt.bestEffort });
    } catch(e){
      if(DEBUG) console.log(`${MYNAME}: convex search failed: ${e.message}`);
    }
    convexSearch = found
      ? { found: found.convex, bestEffort: !found.convex, excess: found.excess, states: found.states,
          solutions: found.solutions, unions: found.unions, exhausted: found.exhausted, geometry: found.geometry }
      : { found: false, bestEffort: false };
    if(found){
      for(let b = 0; b < n; b++){
        const c = found.cells[b];
        cells[b] = { orbit: b, coset: permOfWord(c.word)[0], beta: b, word: c.word, itrans: c.itrans,
                     score: dist0(anchorOf(c.itrans)) };
      }
    }
  }
  ...
  for(let i = 0; i < n; i++){
    for(let s = 0; s < nsides; s++){
      ...
      const side = {
        ...
        neighborClass: j,   // the class of every cell across such a wall, see subgroupDomainTable()
      };
      sides.push(side);
  ...
  return {
    ...
    isConvex:      isUnionConvex({ group, cells }),
    convexSearch:  convexSearch,
...
/*
  ---- the subgroup domain for the shaders --------------------------------------

  Every cell of the tiling of G is h T_j (F) for a unique h in H and a unique
  transversal cell j, its CLASS (the orbit class of buildSubgroupDomain).  The
  classes of the neighbours of a cell depend on its class alone: across side s,
  h T_j t_s (F) has the class next[j][s] of T_j t_s (F).  So a point folded
  into F by the pairing transforms of G can carry the class of its cell along
  (subgroupFold): it starts at 0, the class of F itself, and becomes
  next[class][s] whenever the fold crosses side s.  With the class known,
  T_class maps the folded point into the cell of H's domain the point belongs
  to (subgroupImage): the fold into the domain of H, which needs no convex
  domain of H.  The sides of F at the folded point are the walls of the
  point's cell, of which boundary[class][s] are walls between two H-tiles.

  lib/shaders/overlay_subgroup.glsl.mjs is the shader twin of this section.
*/

/**
  the class table of a subgroup domain

  return {
    n, m:      count of cells and of sides of F
    next:      next[j][s]     class of the cell across side s of a cell of class j
    boundary:  boundary[j][s] true when that wall lies between two H-tiles
  }
*/
export function subgroupDomainTable(domain){

  const n = domain.n;
  const m = domain.genNames.length;
  const next = [], boundary = [];
  for(let j = 0; j < n; j++){
    next.push(new Array(m).fill(0));
    boundary.push(new Array(m).fill(true));
  }
  for(const sd of domain.sides){
    next[sd.cell][sd.side] = sd.neighborClass;
    boundary[sd.cell][sd.side] = (sd.kind === 'boundary');
  }
  return { n, m, next, boundary };
}

/**
  fold a point into F by the pairing transforms of G, following the class of
  the cell of the point (the CPU twin of subFold() of the shaders)

  opt = { group, table, pnt, maxIterations = 500 }

  return { pnt: the folded point, cls: class of the cell of the point,
           itrans: the fold (itrans.transform(opt.pnt) is the folded point),
           inDomain: false when the iterations ran out }
*/
export function subgroupFold(opt){

  const group = opt.group;
  const table = opt.table;
  const fd = group.getFundDomain();
  const rev = group.getReverseITransforms();
  const maxIterations = isDefined(opt.maxIterations) ? opt.maxIterations : 500;

  let pnt = opt.pnt;
  let cls = 0;
  let itrans = new ITransform([], '');
  for(let k = 0; k < maxIterations; k++){
    const i = fd.findIndex(s => U4.sigDistanceSP(s, pnt) > EPS);
    if(i < 0)
      return { pnt, cls, itrans, inDomain: true };
    pnt = rev[i].transform(pnt);
    itrans = itrans.concat(rev[i]);
    cls = table.next[cls][i];
  }
  return { pnt, cls, itrans, inDomain: false };
}

/**
  the image of a point in the domain of H: fold into F with G, then into the
  cell of the class of the point

  return { pnt, cls, itrans (the whole map, an element of H), inDomain }
*/
export function subgroupImage(opt){

  const f = subgroupFold(opt);
  const cell = opt.domain.cells[f.cls];
  return {
    pnt:      cell.itrans.transform(f.pnt),
    cls:      f.cls,
    itrans:   f.itrans.getCopy().concat(cell.itrans),
    inDomain: f.inDomain,
  };
}

/**
  pack the subgroup domain into a data texture array for the shaders: the
  group layout of DataPacking with F's sides as the domain and the cell
  transforms T_j (F -> cell j) as the transforms, followed by the class table,
  one texel per (cell, side): x = next class, y = 1 for a boundary wall.  The
  free components of the header texel hold n (y), m (z) and the offset of the
  table (w).
*/
export function packSubgroupDomain(domain){

  const table = subgroupDomainTable(domain);
  const { n, m } = table;
  const data = DataPacking.packGroupToArray({
    s: domain.fundDomain,
    t: domain.cells.map(c => c.itrans.getRef()),
  });
  const tableOffset = data.length/4;
  const out = new Float32Array(data.length + 4*n*m);
  out.set(data);
  out[1] = n;
  out[2] = m;
  out[3] = tableOffset;
  for(let j = 0; j < n; j++){
    for(let s = 0; s < m; s++){
      ...
      out[k]   = table.next[j][s];
      out[k+1] = table.boundary[j][s] ? 1 : 0;
    }
  }
  return out;
}
...
/**
  upload packSubgroupDomain() into a data sampler (DataPacking.createGroupDataSampler);
  the texture bound to the active unit before the call is bound again after it
*/
export function packSubgroupDomainToSampler(gl, sampler, domain){
