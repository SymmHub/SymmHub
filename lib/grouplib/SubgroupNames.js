...
import { computeFrame, subgroupKey, subgroupClassKey, wallpaperTypeOfKey } from './SubgroupKey.js';
import {
  subgroupsData,
  subgroupStructure,
  presentationOf,
...
/*
  ...
  The type is read off the geometric key of the subgroup
  (SubgroupKey.wallpaperTypeOfKey): its point group, and for each reflection
  whether the coset holds mirrors, glides or both, which the offsets and the
  lattice decide.  That works at any index.  The catalogue defines the type
  by an isomorphism invariant instead, read off a Reidemeister-Schreier
  presentation of the subgroup: the number of subgroup classes it has at each
  index up to GROWTH_DEPTH together with its first homology (sublib's
  subgroupStructure), compared with the same fingerprint of the 17 wallpaper
  groups (growth to depth 6 leaves three ambiguous pairs; homology splits two
  of them, ** vs 22*, *x vs 22x, and depth 7 the last, 3*3 vs 632).  The two
  agree on every subgroup of the manifests; the fingerprint stays as the
  fallback when there is no key, and it fails beyond index 12 or so, where
  the rewritten presentation has too many generators to enumerate.
...
export function makeSubgroupNamer(opt){
  ...
  /** the key result of the entry (subgroupClassKey or subgroupKey), null without one */
  function keyResultOf(entry){
    if(!mGroup) return null;
    try {
      const arg = { group: mGroup, frame: mFrame, cosets: entry.cosets };
      return classInvariant ? subgroupClassKey(arg) : subgroupKey(arg);
    } catch(e){
  ...
  function ensureIndex(index){
    ...
    prepare();
    const t0 = Date.now();
    const infos = data.subgroups.filter(s => s.index === index).map(entry => {
      const kr = keyResultOf(entry);
      // the type from the geometry of the key; the fingerprint when there is none
      const type = (kr && wallpaperTypeOfKey(mFrame, kr)) || typeOf(entry);
      return { entry, type, key: kr ? kr.key : null, name: null };
    });
