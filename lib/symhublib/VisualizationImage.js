...
function VisualizationImage(par={}){
    ...
    function makeParams(cf) {
        ...
        return {
            ...
            imageId:    ParamString({obj: cf, key: 'imageId', name: 'visible images', onChange: oc,
                            tooltip: 'space-separated image ids to composite — leave blank to show every image in the pattern'}),
        }
    } // function makeParams()
    ...
    //
    //  The image ids this layer is set to composite, as an explicit list.
    //  Returns null when imageId is blank — blank means "every image the
    //  pattern has", which this layer cannot enumerate without patternData.
    //  Callers that need to filter by visibility treat null as "no filter".
    //
    function getVisibleImageIds() {
        if (!mConfig.imageId) return null;
        return mConfig.imageId.trim().split(/\s+/).filter(Boolean);
    }
    ...
    function render(par){
       ...
       //if(DEBUG) console.log(`${MYNAME}.render()`, par);
        ...
        // Blank imageId defaults to "every component the pattern currently has"
        // (falls back to [] — the single-buffer path — for patterns that aren't
        // multi-component, e.g. a plain PatternImage).
        const explicitIds = getVisibleImageIds();
        const ids = explicitIds
            ?? (par.patternData?.layers?.[0]?.components ?? []).map(c => c.name);
