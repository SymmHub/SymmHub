...
function VisualizationImage(par={}){
    ...
    function render(par){
       ...
       //if(DEBUG) console.log(`${MYNAME}.render()`, par);
        ...
        //  colormap uniforms 
        //
        // Select buffer: use imageId to pick a named component when patternData is available,
        // otherwise fall back to par.dataBuffer (single-buffer backward compatibility).
        let selectedBuffer = par.dataBuffer;
        if (par.patternData && cmCfg.imageId) {
            const compBuf = par.patternData.getComponentBuffer(cmCfg.imageId);
