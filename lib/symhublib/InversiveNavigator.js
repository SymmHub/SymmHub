    ...
    getUniforms(un, timestamp) {
        ...
        if(this.animatedPointer){
            ...
            if(ap.isMoving()) {
                ...
                if(DEBUG) console.log('pointer:', fa2s(spnt, 2), ' , ', fa2s(oldp, 2),', ',fa2s(ap.getSpeed(),1),fa2s(ap.getForce(),2),timestamp);
                this.onDragEuclidean(spnt);
                this.informListener();
            } else {
                if(DEBUG) console.log('   pointer stopped ',timestamp);
