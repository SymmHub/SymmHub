...
const AP_PARAMS_EUC = {springForce: 200, freeFrictionFactor: 0.05,dragFrictionFactor: 2.};
const AP_PARAMS_HYP = {springForce: 200, freeFrictionFactor: 0.05,dragFrictionFactor: 2.};
const AP_PARAMS_ELL = {springForce: 50, freeFrictionFactor: 0.01,dragFrictionFactor: 2.};
...
export class InversiveNavigator {
    ...
    init(options) {
        ...
        if(options.useAnimatedPointer){
            this.animatedPointer = new AnimatedPointer();
        }
    ...
    setParamsMap(pmap, initialize=false) {
        ...
        if(this.animatedPointer) this.animatedPointer.stop();
    ...
    getUniforms(un, timestamp) {
        ...
        if(this.animatedPointer){
            
            let ap = this.animatedPointer;
            ap.calculate(timestamp*MS); 
            this.timeStamp = timestamp;
            if(ap.isMoving()) {
                let pnt  = ap.getPnt();
                this.onPointerDrag(pnt, ap.eventData, timestamp);
                if(this.pointerData) this.pointerData.log.push({log: 'pointer moving', timestamp: timestamp});
                if(this.pointerData) this.pointerData.pointerX.push({x: timestamp, y: pnt[0]});
                this.informListener();
            } else {
                if(this.pointerData) this.pointerData.log.push({log: 'pointer stopped', timestamp: timestamp});
    ...
    onMouseWheel(evt) {
        ...
        if (evt.ctrlKey) {
            this.onWheelHyperbolic(evt);
        } else if (evt.shiftKey) {
            this.onWheelElliptic(evt);
        } else {
            this.onWheelEuclidean(evt);
        }
        this.informListener();
    ...
    onMouseMove(evt) {
        ...
        //if(DEBUG) console.log('onMouseMove()',fa2s(spnt,2));
        
        if(this.animatedPointer) {           
        
            let ap = this.animatedPointer;
            ap.setMouse(spnt[0], spnt[1]);  
            if(this.pointerData) this.pointerData.mouseX.push({x: this.timeStamp, y: spnt[0]});
            this.informListener(); // initialize the repaint 
            
        } else {
            
            this.onPointerDrag(spnt, evt, 0.);
    ...
    //
    onPointerDrag(spnt, eventData, timestamp){
                
        //let oldp = this.prevPointerPos;        
        if(this.pointerData) this.pointerData.log.push({log:'onPointerDrag():', spnt: fa2s(spnt, 2),timestamp});
    ...
    getPointerParams(evt){
        
        if(evt.shiftKey) 
            return AP_PARAMS_ELL;
        if(evt.ctrlKey) 
            return AP_PARAMS_HYP;
        else 
            return AP_PARAMS_EUC;
    ...
    onMouseDown(evt) {
        ...
        if(this.animatedPointer) { 
            ...
            ap.setParams(this.getPointerParams(evt));
            if(DEBUG) this.pointerData = this.initPointerData();
            if(DEBUG) this.pointerData.log.push({log:'onMouseDown()', evt: evt});
    ...
    //
    //  makes clear pointerData and clears timeout if exists 
    //
    initPointerData(){
        if(this.timeoutId){
    ...
    onDragElliptic(spnt){
        ...
        if(eLength(sub(newPos, oldPos)) < 0.01)
            return;
