...
export function createDocumentManager(options) {
     ...
     * Called when the user double-clicks a JSON file in the FileSelectionDialog.
     */
    ...
    function openSaveAsDialog(suggestedName) {
        ...
        mSaveAsDialog.show({
            ...
            onRootChange: adoptRootFromPicker,
        });
    }
    ...
    function ensureFileDialog() {
        if (!mFileDialog) {
            mFileDialog = createFileSelectionDialog({
                ...
                onFileRenamed:  onDocumentFileRenamed,
                onFileMoved:    onDocumentFileMoved,
            });
        }
        return mFileDialog;
    }

    // ── Renames and moves made in the Files window ────────────────────────────
    //
    //  The current document remembers the folder and the file name it was
    //  loaded from, and Save writes there under the document's name. When
    //  that file is renamed or moved in the Files window the document
    //  follows it; otherwise the next Save would bring the old file back.
    //

    async function onDocumentFileRenamed(oldName, newName, dirHandle) {
        const doc = mCurrentDocument;
        if (!doc || !(await isDocumentOrigin(doc, dirHandle, oldName))) return;
        if(DEBUG)console.log(MYNAME + '.onDocumentFileRenamed():', oldName, '->', newName);
        doc.setOrigin(dirHandle, newName);
        if (doc.setName) doc.setName(newName);
        cbSetTitle(newName);
    }

    async function onDocumentFileMoved(name, fromDirHandle, toDirHandle) {
        const doc = mCurrentDocument;
        if (!doc || !(await isDocumentOrigin(doc, fromDirHandle, name))) return;
        if(DEBUG)console.log(MYNAME + '.onDocumentFileMoved():', name);
        doc.setOrigin(toDirHandle, name);
    }

    /** true when doc was loaded from the file called name in dirHandle */
    async function isDocumentOrigin(doc, dirHandle, name) {
        const originFolder = doc.getOriginFolderHandle?.();
        const originName   = doc.getOriginFileName?.();
        if (!originFolder || originName !== name) return false;
        try {
            return await originFolder.isSameEntry(dirHandle);
        } catch (e) {
            return false;
        }
    }
    ...
    /**
     * A root change made inside FolderPickerDialog — opened from Save As or
     * Export Image — propagates here, so the Documents dialog and every
     * dialog it feeds share one root of permitted folders.
     */
    function adoptRootFromPicker(rootHandle, folderHandle) {
        const existed = !!mFileDialog;
        const fd = ensureFileDialog();
        if (!existed) fd.setVisible(false);
        fd.adoptRoot(rootHandle, folderHandle);
    }
