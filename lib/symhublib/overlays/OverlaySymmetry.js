/*
  ...
  a presentation the table stays empty and `cosets` is typed by hand.  The
  subgroups are listed under the names of the colour group catalogue
  (SubgroupNames.js: G/H[n]#k for a wallpaper group, the sublib ids otherwise).
*/
...
import { makeSubgroupNamer, compareSubgroupNames } from '../../grouplib/SubgroupNames.js';
...
export function makeOverlaySymmetry({ config, onChange, getInitPar, getGLCtx }) {
    ...
    let mTableKey = null;
    let mNamer    = null;    // the names of the table's subgroups (SubgroupNames.js)
    ...
    function ensureTable() {
        ...
        mNamer = mTable ? makeSubgroupNamer({ data: mTable, presentation: pres }) : null;
        refreshChoices();
        return mTable;
    }
    ...
    /** the name a subgroup of the table is listed under */
    ...
    function subgroupChoicesFor(indexChoice) {
        ...
        const names = mTable.subgroups.filter(s => s.index === index).map(s => subgroupName(s));
    ...
    function refreshChoices() {
        ...
        mUI.subgroup = entry ? subgroupName(entry) : SELECT;
        if (mParams) {
    ...
    function applySubgroupChoice() {
        ...
        const entry = mNamer ? mNamer.entryByName(mUI.subgroup)
                             : mTable.subgroups.find(s => String(s.subgroup) === mUI.subgroup);
        if (!entry) return;
