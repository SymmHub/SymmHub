/*
  ...
  A kind is described by a spec:
    ...
    symmetry     true for the kinds drawn from group data: the item gets a
                 `symmetry` folder (the renderer's group or a subgroup of it,
                 see OverlaySymmetry.js) and its own subgroup data (OverlaySymmetry.getUniforms) when a subgroup is used

  ctx gives the hooks access to the item: ctx.config, ctx.params, ctx.glCtx, ctx.initPar,
  and ctx.symmetry, the OverlaySymmetry of a group using kind (null otherwise).
*/
...
import { makeOverlaySymmetry, makeSymmetryConfig } from './OverlaySymmetry.js';
...
export function makeOverlayItem(spec, par = {}) {
    ...
    if (spec.symmetry) mConfig.symmetry = makeSymmetryConfig();
    ...
    const ctx = {
        ...
        get symmetry() { return mSymmetry; },
        onChange,
    };

    // the symmetry the item is drawn with (the renderer's group or a subgroup), group using kinds only
    const mSymmetry = spec.symmetry
        ? makeOverlaySymmetry({ config: mConfig.symmetry, onChange, getInitPar: () => mInitPar, getGLCtx: () => mGLCtx })
        : null;
    ...
    function getParams() {
        if (!mParams) {
            mParams = {
                ...
                ...(mSymmetry ? { symmetry: mSymmetry.makeParams() } : {}),
            };
        }
        return mParams;
    }
    ...
    function init(par) {
        ...
        if (mSymmetry) mSymmetry.init(par);
    ...
    function render(par) {
        ...
        // the symmetry may build its data and upload it into its sampler here: that
        // binds a texture, so it comes before the textures of the pass are bound
        const symUni = mSymmetry ? mSymmetry.getUniforms(par) : null;
        ...
        if (symUni) prog.setUniforms(symUni);   // the subgroup data of the item
    ...
    if (mSymmetry || spec.onGroupChanged) {
        item.onGroupChanged = (group) => {
            if (mSymmetry) mSymmetry.onGroupChanged(group);
            if (spec.onGroupChanged) spec.onGroupChanged(group, ctx);
        };
    }
    if (mSymmetry) item.getSymmetry = () => mSymmetry;
