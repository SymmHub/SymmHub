 ...
 *  - keys the legacy format never had (walls, symmetry, elements, opacity, ...)
 *    are the item's own and pass through unchanged, so a patch by the legacy
 *    key reaches every parameter of the item.
 *
 * @param {string}  key      legacy feature key
...
export function legacyToItemParams(key, value, partial = false) {
    ...
    switch (key) {
        case 'tiling': {
            ...
            passThrough(out, value, ['outline', 'fill']);
            return out;
        }
        case 'fundDomain':
        case 'buffer': {
            ...
            passThrough(out, value, ['fill', 'outline', 'enabled']);
            return out;
        }
        default:
            return clone(value);
...
/** copy the keys of value which are not legacy keys into out: the item's own parameters */
function passThrough(out, value, legacyKeys) {
    for (const k of Object.keys(value)) {
        if (legacyKeys.includes(k)) continue;
        out[k] = isPlainObject(value[k]) ? clone(value[k]) : value[k];
    }
}
