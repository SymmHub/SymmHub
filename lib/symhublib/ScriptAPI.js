 ...
 * @param {() => {width,height}}    hooks.getCanvasSize    — current canvas pixel size
 * @param {(w,h)=>HTMLCanvasElement} hooks.renderToCanvas  — render one frame at an
 *                                                          explicit size, restoring
 *                                                          the previous size after
...
export function createScriptAPI(hooks) {
    ...
    const api = {
        ...
        // Canvas size / offscreen render at an explicit size
        getCanvasSize:    hooks.getCanvasSize,
        renderToCanvas:   hooks.renderToCanvas,

        // Draw a different group (e.g. a subgroup) without touching the maker
        setGroupOverride: hooks.setGroupOverride,
        getGroupOverride: hooks.getGroupOverride,
