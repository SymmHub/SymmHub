 ...
 */
function drawBounds(ctx, cornersScreen, color) {
    ctx.save();
    ctx.beginPath();
 ...
 */
function drawCorners(ctx, cornersScreen, strokeColor) {
    ctx.save();
...
function PatternTransformRenderer(params) {
    ...
    function renderUI(ctx, canvasTransform) {
        ...
        drawBounds(ctx, cornersScreen, COLOR_BOUNDS);
        drawPoint(ctx, pivotScreen[0], pivotScreen[1], PIVOT_COLORS, PIVOT_SIZES);
        drawCorners(ctx, cornersScreen, COLOR_CORNER_STROKE);
        ...
        if (mHandler) {
            ...
            if (fundDomainTrans) {
                ...
                drawBounds(ctx, cornersScreenTransformed, COLOR_RED_POINT);
                drawCorners(ctx, cornersScreenTransformed, COLOR_RED_POINT);
