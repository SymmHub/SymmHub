...
function SymRenderer(options) {
    ...
    ptc.canonicalTrans = getCanonicalPatternTransform(ptc);
    let mPatternTransform = new TransformMotion2D({
        center: [ptc.canonicalTrans.centerX, ptc.canonicalTrans.centerY],
        scale: ptc.canonicalTrans.scale,
        angle: ptc.canonicalTrans.angle * TORADIANS
    });
    ...
    function onPatternPivotChanged(key,value){
        ...
        // pivot of pattern changed. We recalculate patternCenterX, and patternCenterY
        let cfg = mConfig.patternTransConfig;
        upgradePatternPivot(cfg);
        mParams.pattern.patternTransform.centerX.updateDisplay();
        mParams.pattern.patternTransform.centerY.updateDisplay();
    ...
    function makeToolsParams() {
        ...
        return ParamGroup({
            ...
            params: {
                ...
                patternTransform: ParamFunc({func: () => setTool('patternTransform'), name: 'pattern transform'}),
            }
        });
    ...
    function makeToolbox(options={}) {
        ...
        if ( mPattern.canAnimate || mPattern.doStep ) {
    ...
    function renderGLCanvas() {
        ...
        const renderParams = {
                patternTransConfig: ptc.canonicalTrans, 
    ...
    function getRenderUni(){
        ...
        const ptc = mConfig.patternTransConfig;    
        const ptcCanon = ptc.canonicalTrans;
        
        let visConf     = mConfig.misc;
        let symOptions  = mConfig.symmetry.options;
        let simBuffer   = mPattern.getPatternData({patternTransConfig:ptcCanon}).getMainBuffer();

        let ss = 0.5 / ptcCanon.scale; // 0.5 - because texture box originaly has range [0,1]. But drawing images have range [-1,1]
        let sa = ptcCanon.angle * TORADIANS;
        
        let renderUni = {
            uBufScale:      [ss * Math.cos(sa), -ss * Math.sin(sa)],
            uBufCenter:     [ptcCanon.centerX, ptcCanon.centerY],
    ...
    function onPatternTransformChanged() {
        ...
        ptc.canonicalTrans = getCanonicalPatternTransform(ptc);
        const ptcCanon = ptc.canonicalTrans;
        if(DEBUG) console.log(`${MYNAME}.onPatternTransformChanged()`, ptc, ptcCanon);
        mPatternTransform.setParams({
            center: [ptcCanon.centerX, ptcCanon.centerY],
            scale: ptcCanon.scale,
            angle: ptcCanon.angle * TORADIANS
        });
        updateTransGroup();
        scheduleRepaint();
    ...
    function updateTransGroup() {
        ...
        let tr3 = ITransform.getScale(1 / ptcCanon.scale);
        let tr2 = ITransform.getRotation([0, 0, 1], -TORADIANS * ptcCanon.angle);
        let tr1 = ITransform.getTranslation([-ptcCanon.centerX, -ptcCanon.centerY]);
        ...
        if(DEBUG) console.log(`${MYNAME}.updateTransGroup()`, mTransGroup);
    ...
    function getPlotData(dataSliceY, plotType) {
        ...
        const ptcCanon = mConfig.patternTransConfig.canonicalTrans;
        let simBuffer = mPattern.getPatternData({patternTransConfig:ptcCanon}).getMainBuffer();
