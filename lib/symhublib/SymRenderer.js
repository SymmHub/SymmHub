...
const progIsolines = {
    ...
    vs: baseVertexShader,
    fs: {
        frags: [fragIsplane, fragInversiveSampler, fragComplex, fragTexUtils,
                fragProjection, 
                fragIsoUtil, fragIsoMain]
    }
};
...
let gPrograms = [
    ...
    progBufferToScreenBumpmap,
    progCopy,
    progBufferVisColormap,
    progBufferVisHeightmap,
    progBufferVisTextured,
    progIsolines
...
function SymRenderer(options) {
    ...
    function makeAppName(){
        if(DEBUG)console.log('${MYANME}.makeAppName() mGroupMaker: ', mGroupMaker);
    ...
    function makeColormapParams() {
        ...
        return ParamGroup({
            ...
            params: {
                ...
                transparency: ParamFloat({obj: cf, key: 'transparency', min: 0, max: 1, step: 0.001, onChange: oc})
            }
        });
    ...
    function makeIsolinesParams() {
        ...
        let cf = mConfig.visualization.isolines;
        let oc = scheduleRepaint;
        ...
        return ParamGroup({
            ...
            params: {
                ...
                step:    ParamFloat({obj:cf, key: 'step',   min:mi, max: ma, step: inc, onChange: oc}),
                offset:  ParamFloat({obj:cf, key: 'offset',  min:mi, max: ma, step: inc, onChange: oc}),
                thickness:  ParamFloat({obj:cf, key: 'thickness',  min:-1, max: 100, step: 0.1, onChange: oc}),
                levels:   ParamInt({obj:cf, key: 'levels',  min:1, max: 6, onChange: oc}),
                color:    ParamColor({obj: cf, key: 'color',  onChange: oc}),
            }
        });
    ...
    function renderGLCanvas() {
        ...
        if(mConfig.visualization.isolines.enabled)
            renderIsolines();
    ...
    //
    //  returns common usinforms used for symmetric rendering 
    //
    ...
    //
    function getStandardTexTransUni(cnv){
    ...
    //
    //  render visualization using texture image for coloring the UV values
    //
    function renderVisTexture() {
        ...
        if(visOpt.useMipmap){ // create mipmap 
            ...
            let progVis = progBufferVisTextured.program;
            progVis.bind();
        ...
        let renderProg = progBufferToScreenTextured.program;
        renderProg.bind();
    ...
    function renderIsolines() {
        
        let iconf = mConfig.visualization.isolines;
        
        enableBlending();                     
        let prog = progIsolines.program;
        prog.bind();
        
        let ctUni = mNavigator.getUniforms({}, mTimeStamp);
        prog.setUniforms(ctUni);
        let renderUni = getRenderUni();       
        prog.setUniforms(renderUni);

        let isoUni = {
            uIsolinesColor: premultColorArray(hexColorToArray(iconf.color)),
            uIsoStep:       iconf.step,
            uIsoOffset:     iconf.offset,
            uIsoThickness:  iconf.thickness,
            uIsoLevels:     iconf.levels,
            
        }
        prog.setUniforms(isoUni);

        setViewport(mCanvas.glCanvas);
    ...
    function renderVisColormap() {
        ...
        let colormapUni = { 
            ...
            uMinValue:      visConf.minValue,
            uMaxValue:      visConf.maxValue,
            uColormapTransparency: visConf.transparency,
        };
    ...
    function renderVisBumpmap() {
        ...
        if(visOpt.useMipmap){ // create mipmap of grayscale image of data                    
            
            let progVis = progBufferVisHeightmap.program;
            progVis.bind();
        ...
        let renderProg = progBufferToScreenBumpmap.program;
