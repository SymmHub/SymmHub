...
// How close to the hyperbolic bounding circle (|p| = 1) a dragged corner is
// allowed to get. Corners of the box being dragged are clamped to |p| <= this.
const DRAG_BOUND_FRACTION = 0.95;
...
function SymRenderer(options) {
    ...
    const mScripting = createSymRendererScripting({
        ...
        renderToCanvas:   (w, h) => renderToCanvas(w, h),
        setGroupOverride: (g) => setGroupOverride(g),
        getGroupOverride: () => getGroupOverride(),
        // Toolbar injection
        getToolbox:       () => mToolbox,
        resPath:          resPath,
    ...
    const myself = {
        ...
        // present only once the pattern-transform tool has been activated
        getPatternTransformHandler: () => mPatternTransformHandler,
    };
    ...
    // Optional: (gui, params) => void — replaces the default top-level layout
    // (createParamUI(gui, params)) built by makeParams(). Lets one app lay out
    // its own menu while every Param object underneath (and so serialization,
    // presets, scripting) stays exactly what makeParams() already builds.
    let mMenuBuilder = options.menuBuilder || null;
    ...
    let mOrbifoldEdgeRenderer = OrbifoldEdgeGLRenderer({
        gl: mGLCtx.gl
    }); // draws OrbifoldUITool's fundamental-domain edges directly on the GL canvas
    ...
    function updateGroupUITool() {
        mGroupUITool = (mGroupMaker && mGroupMaker.createUITool)
            ? mGroupMaker.createUITool({
                ...
                glEdgeRenderer: mOrbifoldEdgeRenderer,
            })
            : null;
    }
    updateGroupUITool();
    ...
    // When set, this group is used instead of the group maker's own.  Batch /
    // scripted rendering uses it to draw a subgroup (its fundamental domain,
    // tiling and pairing transforms) without disturbing the maker or the UI.
    let mGroupOverride = null;
    ...
    function startApplication() {
        ...
        mVisualization.init({glCtx: mGLCtx, onChange:scheduleRepaint, getGroupPresentation: getGroupPresentation});
        if (DEBUG)
    ...
    function initGUI() {
        ...
        if (mMenuBuilder) mMenuBuilder(mGUI, getParams());
        else               createParamUI(mGUI, getParams());
    ...
    //
    //  Use the given group instead of the group maker's.  Pass null to go back
    //  to the maker.  The symmetry transform is still applied on top, exactly
    //  as it is for a maker supplied group.
    //
    function setGroupOverride(group) {
        mGroupOverride = group || null;
        onGroupChanged();
    }

    function getGroupOverride() {
        return mGroupOverride;
    }
    ...
    function onGroupChanged() {
        ...
        if (mVisualization.onGroupChanged) mVisualization.onGroupChanged(mGroup);
        scheduleRepaint();
    ...
    function prepareGroupData(){
        ...
        mGroup = mGroupOverride ? mGroupOverride : mGroupMaker.getGroup();
        ...
        case TOOL_PATTERN_TRANSFORM:
            ...
            if (!mPatternTransformHandler) {
                mPatternTransformHandler = PatternTransformHandler({
                    ...
                    getGroup: getGroup,
                    // outline/handles follow the union of the *visible* image
                    // boxes; null (no multi-image pattern) => the default [-1,1]^2 box
                    getPatternBox: computePatternUnionBox,
                    // live get/set handles for each visible texture image, so the
                    // tool can select and drag individual images
                    getImageTransforms: getVisibleImageTransforms,
                    // in the hyperbolic plane, keep every dragged corner off
                    // the bounding circle
                    getMaxRadius: getDragMaxRadius,
                });
            }
            if (!mPatternTransformRenderer) {
                mPatternTransformRenderer = PatternTransformRenderer({
                    // the tool's UI is drawn in WebGL, straight onto the
                    // pattern canvas — nothing of it goes to the 2D overlay
                    gl: mGLCtx.gl,
                    config: mConfig.patternTransConfig,
                    handler: mPatternTransformHandler,
                    getPatternBox: computePatternUnionBox,
                    // patterns that own multiple images expose their per-image
                    // texture boxes here (visible ones only); others return
                    // nothing and nothing extra is drawn
                    getExtraBoxes: getVisibleImageBoxes,
                });
            }
    ...
    //
    //  Axis-aligned union of every image box, expressed in pattern/buffer
    //  space (the space per-image transforms live in). Returned as
    //  {minX,minY,maxX,maxY}; null when the pattern has no image boxes, so
    //  callers fall back to the plain [-1,1]^2 pattern box.
    //
    //
    //  The image ids currently displayed by the visualization, or null for
    //  "no filter". Used so the transform tool only ever shows and grabs the
    //  images you can actually see (a hidden image has no box and no handles).
    //
    function getVisibleImageIds() {
        return (mVisualization && mVisualization.getVisibleImageIds)
            ? mVisualization.getVisibleImageIds()
            : null;
    }

    //
    //  Keep only the entries whose id is visible. Entries are {id,...} from
    //  getImageBoxes()/getImageTransforms(). A null id filter means keep all.
    //
    function filterToVisible(entries) {
        if (!entries || !entries.length) return entries;
        const visible = getVisibleImageIds();
        if (visible === null) return entries;
        const keep = new Set(visible);
        return entries.filter(e => keep.has(e.id));
    }

    function getVisibleImageBoxes() {
        const boxes = (mPattern && mPattern.getImageBoxes) ? mPattern.getImageBoxes() : null;
        return filterToVisible(boxes);
    }

    function getVisibleImageTransforms() {
        const trs = (mPattern && mPattern.getImageTransforms) ? mPattern.getImageTransforms() : null;
        return filterToVisible(trs);
    }

    //
    //  Largest |p| (world/math coords) a dragged box corner may reach, or 0 for
    //  "no limit". The hyperbolic plane is drawn in the Poincare unit disk, so
    //  the picture has to stay strictly inside the bounding circle |p| = 1 —
    //  nothing may come closer to it than DRAG_BOUND_FRACTION. Euclidean and
    //  spherical groups (and every GroupMaker that doesn't report a curvature)
    //  are unbounded, so they get no limit.
    //
    function getDragMaxRadius() {
        const k = (mGroupMaker && mGroupMaker.getCurvature) ? mGroupMaker.getCurvature() : undefined;
        return (typeof k === 'number' && k < 0) ? DRAG_BOUND_FRACTION : 0;
    }

    function computePatternUnionBox() {
        const boxes = getVisibleImageBoxes();
        if (!boxes || !boxes.length) return null;

        let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
        const us = [[-1, 1], [1, 1], [1, -1], [-1, -1]];
        for (const b of boxes) {
            const a  = (b.angle || 0) * TORADIANS;
            const s  = (b.scale === undefined ? 1 : b.scale);
            const ca = Math.cos(a), sa = Math.sin(a);
            for (const u of us) {
                const x = (b.centerX || 0) + s * (ca * u[0] - sa * u[1]);
                const y = (b.centerY || 0) + s * (sa * u[0] + ca * u[1]);
                if (x < minX) minX = x;
                if (x > maxX) maxX = x;
                if (y < minY) minY = y;
                if (y > maxY) maxY = y;
    ...
    //
    //  Render one frame at an explicit pixel size and return a detached copy of
    //  the result, then restore the previous canvas size.
    //
    //  mIsExporting is raised for the duration so that resizeCanvases() and the
    //  animation loop leave the temporary size alone - the same guard the tiled
    //  image export uses.  The pixels are copied into a plain 2d canvas right
    //  after the draw call, so the caller can encode them at leisure without
    //  depending on the WebGL drawing buffer surviving compositing.
    //
    //  Used by scripted / batch rendering.  Returns an HTMLCanvasElement.
    //
    function renderToCanvas(width, height) {

        const orig = getCanvasSize();
        const wasExporting = mIsExporting;
        let out = null;
        ...
        try {
            ...
            out.width  = width;
            out.height = height;
            out.getContext('2d').drawImage(mCanvas.glCanvas, 0, 0);
        } finally {
            setCanvaSize(orig.width, orig.height);
            mIsExporting = wasExporting;
            renderFrame();
        }
        return out;
    }
    ...
    //
    //  presentation (generators and relators) of the current group as the group
    //  maker gives it, null when the maker has none: the subgroup code then
    //  works from its catalogue of presentations
    //
    function getGroupPresentation(){
        if(mGroupOverride) return null;
        return (mGroupMaker && mGroupMaker.getPresentation) ? mGroupMaker.getPresentation() : null;
    }
