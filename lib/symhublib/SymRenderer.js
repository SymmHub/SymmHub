...
function SymRenderer(options) {
    ...
    function onSaveImage() {
        ...
        mDocManager.openExportImageDialog({
            ...
            onSave: (chosenName, folderHandle, exportWidth, exportHeight) => {
                mIsExporting = true;
                const origGlWidth = mCanvas.glCanvas.width;
                const origGlHeight = mCanvas.glCanvas.height;
                const origOverlayWidth = mCanvas.overlay.width;
                const origOverlayHeight = mCanvas.overlay.height;

                mCanvas.glCanvas.width = exportWidth;
                mCanvas.glCanvas.height = exportHeight;
                mCanvas.overlay.width = exportWidth;
                mCanvas.overlay.height = exportHeight;
                ...
                Promise.all([
                    mImageSaver.saveImageTo(
                        mCanvas.glCanvas,
                        folderHandle,
                        chosenName,
                        null
                    ),
                    mDocManager.writeDocumentToFolder(folderHandle, chosenName + '.png'),
                    tmbPromise
                ]).then(() => {
                    mRendererConfig.exportWidth = exportWidth;
                    mRendererConfig.exportHeight = exportHeight;

                    mCanvas.glCanvas.width = origGlWidth;
                    mCanvas.glCanvas.height = origGlHeight;
                    mCanvas.overlay.width = origOverlayWidth;
                    mCanvas.overlay.height = origOverlayHeight;
