...
/**
 * TODO: rename as RenderingManager or something
 */
function SymRenderer( renderingLayer, options) {
    ...
    const mRenderingLayer = renderingLayer;
    ...
    if (DEBUG)console.log(`${MYNAME} (application: ${mRenderingLayer.getName()})`);
    ...
    function makeAppName(){
        ...
        return [MYNAME,mGroupMaker.getClassName(),mRenderingLayer.getClassName(), mNavigator.getClassName()].join('.');
    }
    ...
    function startApplication() {
        ...
        mRenderingLayer.init( mGLCtx, scheduleRepaint );
        mVisualization.init({glCtx: mGLCtx, onChange:scheduleRepaint});
        if (DEBUG)
            console.log(`${MYNAME} mRenderingLayer: ${mRenderingLayer.getName()}`);
        ...
        //toggleGUI();
        mRenderingLayer.setGroup(mGroup);
        onCanvasSize();
        mDrawingToolRenderer.init({
            simulation: mRenderingLayer,  // TODO: change this somehow
            repainter: {
    ...
    function makeToolbox() {
        ...
        let btnCount = 7;
        toolboxButtons.tools       = createImgBtn({title:'options',   src:resPath('btn_options.svg'),action:toggleGUI});
        toolbox.appendChild(toolboxButtons.tools);

        const layerButtons = mRenderingLayer .getToolboxButtons();
        for (const button of layerButtons) {
            if (isDefined(button)) {
                toolboxButtons[button.name] = createImgBtn({
                    title: button.title,
                    src: resPath( button.src ),
                    action: button.action
                });
                toolbox.appendChild(toolboxButtons[button.name]);
                btnCount++;
            }
        }

        toolboxButtons.recording   = createImgBtn({title:'recording', src:resPath('btn_record_start.svg'),action:onToggleRecording});
        toolboxButtons.download    = createImgBtn({title:'save image',src:resPath('btn_download.svg'),action:onSaveImage});
        toolboxButtons.full_screen = createImgBtn({title:'full screen',src:resPath('btn_full_screen.svg'),action:onFullScreen});
        toolboxButtons.move        = createImgBtn({title:'move',      src:resPath('btn_move2.svg'),action:()=>setTool('move')});
        toolboxButtons.draw        = createImgBtn({title:'draw',      src:resPath('btn_draw.svg'),action:()=>setTool('draw')});
        toolboxButtons.pick        = createImgBtn({title:'pick',      src:resPath('btn_pick.svg'),action:()=>setTool('pick')});
        let space       = createImgBtn({                   src:resPath('btn_vbar.svg')});
        space.style.width = '20px';
        toolbox.appendChild(toolboxButtons.recording);
        toolbox.appendChild(toolboxButtons.download);
        toolbox.appendChild(toolboxButtons.full_screen);
        
        toolbox.appendChild(space);
        
        toolbox.appendChild(toolboxButtons.move);
        toolbox.appendChild(toolboxButtons.draw);
        toolbox.appendChild(toolboxButtons.pick);
    ...
    function renderGLCanvas() {
        ...
        mVisualization.render({
                dataBuffer:     mRenderingLayer.getBuffer(), 
    ...
    function getRenderUni(){
        ...
        let simBuffer   = mRenderingLayer.getBuffer();
    ...
    function onRepaint() {
        ...
        if (mRecorder.isRecording()) {
            ...
            if (mRecorder.isReady()) {
                ...
                const repaintRequired = renderFrame();                
                mRecorder.appendFrame(mCanvas.glCanvas);
                mNeedRepaint = mNeedRepaint || repaintRequired;
    ...
    function renderFrame() {
        ...
        mRenderingLayer.renderFrame( {gl:mGLCtx.gl, animationTime: mConfig.recording.animTime} );
    ...
    function updateTransGroup() {
        ...
        mRenderingLayer.setGroup(mTransGroup);
    ...
    function startRenderingLoop() {
        if(DEBUG)console.log(`${MYNAME}.startRenderingLoop(): `, getTime());
        requestAnimationFrame(onRepaint);
        scheduleRepaint();
    }
    ...
    function onPreparePlotData() {
        ...
        //let plotData = mRenderingLayer.getPlotData();
    ...
    function onToggleRecording() {
        ...
        if (mRecorder.isRecording()) {
            ...
            if (isDefined(mToolbox.recording))
                mToolbox.recording.src = IMG_RECORDING_START;
            mRenderingLayer .recordingToggled( false );
            ...
            mRenderingLayer .recordingToggled( true );
        }
    }
