        ...
        case TOOL_PATTERN_TRANSFORM:
            ...
            if (!mPatternTransformHandler) {
                mPatternTransformHandler = PatternTransformHandler({
                    ...
                    getGroup: getGroup,
                    // outline/handles follow the union of the image boxes;
                    // null (no multi-image pattern) => the default [-1,1]^2 box
                    getPatternBox: computePatternUnionBox,
                    // live get/set handles for each texture image, so the tool
                    // can select and drag individual images
                    getImageTransforms: () => (mPattern && mPattern.getImageTransforms ? mPattern.getImageTransforms() : null),
                });
            }
            if (!mPatternTransformRenderer) {
                mPatternTransformRenderer = PatternTransformRenderer({
                    ...
                    // texture boxes here; others return nothing and nothing extra is drawn
                    getExtraBoxes: () => (mPattern && mPattern.getImageBoxes ? mPattern.getImageBoxes() : null),
                });
            }
