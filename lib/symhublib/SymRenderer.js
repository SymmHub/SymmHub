...
function SymRenderer(options) {
    ...
    function startApplication() {
        ...
        if(useSimpleUI){
            mSamplesSelector.setVisible(false);
    ...
    function makeParams() {
        ...
        const params = {
            ...
            //simulation:     ParamObj({name: 'simulation', obj: mPattern}),
    ...
    function makeSimTransformParams() {
        ...
        return ParamGroup({
            ...
            params: {
                simCenterX: ParamFloat({obj:conf, key: 'centerX', min: -10, max: 10, step: inc, name: 'center X', onChange: onc}),
                simCenterY: ParamFloat({obj:conf, key: 'centerY', min: -10, max: 10, step: inc, name: 'center Y', onChange: onc}),
                simScale:   ParamFloat({obj:conf, key: 'scale',  min: 0.0001, max: 100, step: inc, name: 'scale', onChange: onc}),
                simAngle:   ParamFloat({obj:conf, key: 'angle', min: -360, max: 360, step: inc, name: 'angle(deg)', onChange: onc}),
            }
        });
    } // function makeSimTransformParams(){
    ...
    function makeToolbox(options={}) {
        ...
        buttons.download    = createImgBtn({title:'save image',src:resPath('btn_download.svg'),action:onSaveImage});
    ...
    function renderGLCanvas() {
        ...
        const renderParams = {
                patternTransConfig:mConfig.patternTransConfig, 
                timeStamp: mTimeStamp
                };
        mVisualization.render({
                dataBuffer:     mPattern.getSimBuffer(renderParams), 
    ...
    function getRenderUni(){
        ...
        let simBuffer   = mPattern.getSimBuffer({patternTransConfig:ptc});
        ...
        let renderUni = {
            ...
            //uInterpolation: 0 , getInterpolationId(visConf.interpolation),
    ...
    function getPlotData(dataSliceY, plotType) {
        ...
        let simBuffer = mPattern.getSimBuffer({patternTransConfig:mConfig.patternTransConfig});
    ...
    function setDocumentData(jsonObj){
        ...
        if(false)console.log(`${MYNAME}.setDocumentData() upgraded obj: `, JSON.stringify(jsonObj, null, 2));        
        if(mCanvas.intWin) mCanvas.intWin.setTitle(mCurrentDocument.getName());
        setTool(mConfig.tools.toolName);
    }
