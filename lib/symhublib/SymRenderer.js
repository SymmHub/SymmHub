...
function SymRenderer(options) {
    ...
    let mUseBinaryFiles = !!options.useBinaryFiles;
    ...
    function startApplication() {
        ...
        mCurrentDocument = mDocHandler.createDocument({appInfo: mAppInfo, params:mParams, thumbMaker: getThumbMaker(), useBinaryFiles: mUseBinaryFiles});
    ...
    function getPlotData(dataSliceY, plotType) {
        ...
        let simBuffer = mPattern.getPatternData({patternTransConfig:mConfig.patternTransConfig}).getMainBuffer();
    ...
    async function setDocumentData(jsonObj, binBuffer = null, baseUrl = null){
        ...
        if (jsonObj.binary_data) {
            ...
            if (buf) mCurrentDocument.beginLoad(new BinaryLoader(jsonObj.binary_data, buf));
        }
    ...
    async function readParamsUrl(jsonUrl) {
        ...
        mCurrentDocument = mDocHandler.createDocument({appInfo: mAppInfo, params:mParams, thumbMaker: getThumbMaker(), name: docName, useBinaryFiles: mUseBinaryFiles});
    ...
    async function readParamsFile(jsonFile, binFile = null){
        ...
        // Update the document name (same as readParamText does)
        if(jsonObj.name && mCurrentDocument.setName) mCurrentDocument.setName(jsonObj.name);
        if(mCanvas.intWin) mCanvas.intWin.setTitle(jsonObj.name || jsonFile.name);
