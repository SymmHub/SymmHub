...
const VIS_MIPMAP = 'mipmap';

const RENDER_STYLES = [VIS_COLORMAP, VIS_BUMPMAP, VIS_TEXTURE,VIS_MIPMAP];
...
const fragBufferVisualization = {
    obj: SF,
    id: 'bufferVisualization'
};
...
const progBufferVisualization = {
    name: 'BufferToScreen',
    vs: baseVertexShader,
    fs: {
        frags: [fragColormap, fragBufferVisualization]
    }
};
...
function SymRenderer(options) {
    ...
    function makeVisOptionsParams() {
        ...
        return ParamGroup({
            ...
            params: {
                ...
                interpolation:  ParamChoice({obj: visConf,key: 'interpolation',choice: INTERP_NAMES,onChange: onc}),
    ...
    function drawSimColormap() {
        ...
        //let ctUni = mCanvasTransform.getUniforms({});
        let ctUni = mNavigator.getUniforms({}, mTimeStamp);
        //console.log(`${MYNAME} ctUni: `, ctUni);
    ...
    //
    //  draw simulation using colormap with mipmap
    //
    function drawSimMipmap() {
        ...
        //let canvasmRenderBuffer
        
        //gl.viewport(0, 0, canvas.width, canvas.height);
        //let target = null; // canvas
        mMipmapBuffer.attach(0);
        gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER, mMipmapBuffer.fbo);        
        let c = hexToPremult(mConfig.display.backgroundColor);
        //console.log('c:', c);
        gl.clearColor(c[0], c[1], c[2], c[3]);

        //let cnv = mCanvas.overlay;
        let cnv = mMipmapBuffer;
        gl.viewport(0, 0, cnv.width, cnv.height);
        gl.clear(gl.COLOR_BUFFER_BIT);
        
        
        let progVis = progBufferVisualization.program;
        progVis.bind();
        
        let transUni = { // transformation to render to coloerd buffer 
        ...
        //console.log('cmUni:', cmUni);
        
        progVis.setUniforms(transUni);
        progVis.setUniforms(cmUni);
        //progVis.blit(null); //mMipmapBuffer
        progVis.blit(mMipmapBuffer); //mMipmapBuffer
        
        mMipmapBuffer.attach(0); // set as the current texture 
        ...
        let renderUni = {
            ...
            uSymmetry: symConf.options.useSymm,
            uUseMipmap: true,
            uMipmapData: mMipmapBuffer.texture,
        };
            
 
         let renderProg = progBufferToScreen.program;
