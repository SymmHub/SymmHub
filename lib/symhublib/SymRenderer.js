...
function SymRenderer(options) {
    ...
    function makePatternTransformParams() {
        ...
        return ParamGroup({
            ...
            params: {
                centerX: ParamFloat({obj:conf, key: 'centerX', min: -10, max: 10, step: inc, name: 'center X', onChange: onc, listen: true}),
                centerY: ParamFloat({obj:conf, key: 'centerY', min: -10, max: 10, step: inc, name: 'center Y', onChange: onc, listen: true}),
                scale:   ParamFloat({obj:conf, key: 'scale',  min: 0.0001, max: 100, step: inc, name: 'scale', onChange: onc, listen: true}),
                angle:   ParamFloat({obj:conf, key: 'angle', min: -360, max: 360, step: inc, name: 'angle(deg)', onChange: onc, listen: true}),
            }
        });
    } // function makePatternTransformParams(){
    ...
    function setTool(toolName) {
        ...
        case TOOL_DRAW:
            ...
            break;
        case TOOL_PATTERN_TRANSFORM:
            mCanvas.overlay.style.cursor = [CUR_PATTERN_TRANSFORM, 'move'];
            if (!mPatternTransformHandler) {
                mPatternTransformHandler = PatternTransformHandler({
                    config: mConfig.patternTransConfig,
                    onChanged: onSimTransChanged
                });
            }
            mEventHandler = mPatternTransformHandler;
            break;
        }
