...
export function SymRendererUpgradeData(data, args){
    ...
    upgradePattern(data);
    // 'leftCoset' param of the color symmetry layers was renamed 'useOrbit';
    // the values carry over unchanged: true = coloring by orbits,
    // false = coloring by cosets
    renameKeyDeep(data, 'leftCoset', 'useOrbit');
}

//
// rename a key wherever it appears in the document tree
// (used for straight param renames, value is kept as is)
//
function renameKeyDeep(obj, oldKey, newKey, mapValue){

    if(obj === null || typeof obj !== 'object') return;
    if(Array.isArray(obj)){
        obj.forEach(item => renameKeyDeep(item, oldKey, newKey, mapValue));
        return;
    }
    if(Object.prototype.hasOwnProperty.call(obj, oldKey) && !(newKey in obj)){
        obj[newKey] = mapValue ? mapValue(obj[oldKey]) : obj[oldKey];
        delete obj[oldKey];
    }
    for(const key of Object.keys(obj)){
        renameKeyDeep(obj[key], oldKey, newKey, mapValue);
    }
}
